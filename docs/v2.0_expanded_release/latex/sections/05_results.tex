...
\input{tables/table2_threshold_sweep}

The sweep shows a faithfulness--fluency trade-off (\cref{tab:threshold_sweep}). Peak faithfulness (\textbf{93.37\%}) occurs at $T_{min}=0.10$, our default in later tables. A light threshold catches clearly ungrounded sentences while still allowing medical paraphrase. At $T_{min}=0.25$, faithfulness falls to 89.49\% as retries produce stiffer, less readable text. At $T_{min}=0.50$, faithfulness recovers to 92.41\% largely through near-verbatim copying from WHO passages, which raises faithfulness scores but reduces natural summarization. Answer relevancy also drops slightly at $T_{min}=0.10$ relative to the unguarded baseline (57.81\% vs.\ 61.17\%), consistent with stricter grounding.
...
\input{tables/table3_telemetry}

\Cref{tab:telemetry} summarizes runtime behavior on the same $N=100$ run. Naive and hybrid baselines always stop after one generation pass. X-CDS averages \textbf{1.10 attempts} per query, with a \textbf{90.00\% first-pass validation rate}, so retries are infrequent.

Abstention rises from 10.00\% (naive) and 12.00\% (hybrid) to \textbf{14.00\%} under X-CDS when retrieved passages do not support a direct answer. Mean citations per answer increase from 2.68 to \textbf{3.09}, which reflects more explicit source anchoring rather than longer answers alone.
...
\input{tables/table4_qualitative}
...
where $A$ is the number of generation attempts, $I_a$ is the input prompt size in tokens for attempt $a$, and $O_a$ is the generated output size in tokens. For a representative consult, input prompts average $\sim3,200$ tokens (query + retrieved literature context + graph guidelines) costing $\$0.00024$ USD, and output responses average $\sim350$ tokens costing $\$0.000105$ USD, totaling \textbf{\$0.000345 USD (approx.\ 0.029 INR)} per query at published Vertex AI rates for \texttt{gemini-3.5-flash}.

\subsection{Retrieval Reranking Optimization}
On $N=20$ held-out queries, replacing \texttt{ms-marco-MiniLM-L-6-v2} with \texttt{BAAI/bge-reranker-v2-m3} raised mean context precision from \textbf{66.86\%} to \textbf{69.38\%} (+2.52 points). We kept MiniLM for the main $N=100$ end-to-end runs to limit API cost.
