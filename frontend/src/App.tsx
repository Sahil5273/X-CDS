...
export default function App() {
  ...
  function handleRunLive() {
    const liveQuery = "A patient presents with acute onset of high fever, maculopapular rash, and severe joint pain after travel to India. What NSAID risk must be considered if this is Dengue?";
    setQuery(liveQuery);
    void handleSubmit(liveQuery);
  }
  ...
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col px-4 py-6 sm:px-6 lg:px-8 lg:py-8 pb-16">
      ...
      <main className="flex flex-col flex-1 gap-5">
        {/* Mobile Tab Bar */}
        {result && (
          <div className="flex border border-[var(--line)] lg:hidden bg-white/40 rounded-xl p-1 gap-1">
            <button
              onClick={() => setActiveTab("answer")}
              className={`flex-1 py-2 text-xs font-bold text-center rounded-lg transition ${
                activeTab === "answer"
            ...
            >
              Evidence Sources [{result.contexts.length}]
            </button>
          </div>
        )}
              ...
              <div className="mt-6 border-t border-[var(--line)] pt-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  ...
                  <div className="rounded-xl border border-[var(--line)] bg-white/40 p-3">
                    ...
                    <span className={`mt-1 inline-flex items-center gap-1.5 text-sm font-bold ${result.generation_attempts === 1 && result.validation_passed ? "text-green-600" : "text-amber-600"}`}>
                      <span className={`h-2 w-2 rounded-full ${result.generation_attempts === 1 && result.validation_passed ? "bg-green-600" : "bg-amber-600"}`} />
                      {result.generation_attempts === 1 && result.validation_passed ? "PASSED" : "FAILED"}
                    </span>
                  </div>
                  <div className="rounded-xl border border-[var(--line)] bg-white/40 p-3">
                    <span className="block text-[10px] font-semibold text-[var(--muted)] uppercase tracking-wider">
                    ...
                    <span className={`mt-1 inline-flex items-center gap-1.5 text-sm font-bold ${isAbstention ? "text-amber-600" : "text-green-600"}`}>
                      <span className={`h-2 w-2 rounded-full ${isAbstention ? "bg-amber-600" : "bg-green-600"}`} />
